...
ZTEST(embr_inference,
      test_begin_audio_capture_after_wos_idempotent_when_capturing_success) {
  int ret = embr_inference_begin_audio_capture_after_wos();
  zassert_equal(ret, 0, "first begin should return 0");
  zassert_equal(mock_kernel_wrap_sem_take_forever_call_count, 1,
                "kernel_wrap_sem_take_forever should be called once");
  zassert_equal(mock_embr_app_led_toggle_on_capture_start_call_count, 1,
                "embr_app_led_toggle_on_capture_start should be called once");
  ...
  ret = embr_inference_begin_audio_capture_after_wos();
  zassert_equal(ret, 0, "second begin should return 0");
  zassert_equal(mock_kernel_wrap_sem_take_forever_call_count, 1,
                "kernel_wrap_sem_take_forever should not be called again");
  zassert_equal(
      mock_embr_app_led_toggle_on_capture_start_call_count, 1,
      "embr_app_led_toggle_on_capture_start should not be called again");
  zassert_equal(mock_thingy53_mic_start_call_count, 1,
                "thingy53_mic_start should not be called again");
}

ZTEST(embr_inference, test_begin_audio_capture_after_wos_sync_error_fail) {
...
ZTEST(embr_inference, test_begin_audio_capture_after_wos_led_error_fail) {
  mock_embr_app_led_toggle_on_capture_start_return_value = -EIO;

  int ret = embr_inference_begin_audio_capture_after_wos();
  zassert_equal(
      ret, -EIO,
      "embr_inference_begin_audio_capture_after_wos should return -EIO");
  zassert_equal(mock_kernel_wrap_sem_take_forever_call_count, 1,
                "kernel_wrap_sem_take_forever should be called once");
  zassert_equal(mock_embr_app_led_toggle_on_capture_start_call_count, 1,
                "embr_app_led_toggle_on_capture_start should be called once");
...
ZTEST(embr_inference, test_begin_audio_capture_after_wos_mic_start_error_fail) {
  mock_thingy53_mic_start_return_value = -EIO;
...
ZTEST(embr_inference,
      test_begin_audio_capture_after_wos_retries_after_mic_start_failure_success) {
  mock_thingy53_mic_start_return_value = -EIO;
  ...
  zassert_equal(ret, 0,
                "embr_inference_begin_audio_capture_after_wos should return 0");
  zassert_equal(mock_kernel_wrap_sem_take_forever_call_count, 2,
                "kernel_wrap_sem_take_forever should be called twice");
  zassert_equal(mock_embr_app_led_toggle_on_capture_start_call_count, 2,
                "embr_app_led_toggle_on_capture_start should be called twice");
  zassert_equal(mock_thingy53_mic_start_call_count, 2,
                "thingy53_mic_start should be called twice");
}
...
ZTEST(embr_inference, test_move_slice_unaligned_size_returns_einval_fail) {
  ...
  size_t unaligned_size = (CONFIG_VM3011_PDM_BUFFER_SIZE * sizeof(int16_t)) - 1;
...
ZTEST(embr_inference, test_move_slice_propagates_add_data_error_fail) {
  static int16_t pcm[CONFIG_VM3011_PDM_BUFFER_SIZE];
  size_t pcm_size = CONFIG_VM3011_PDM_BUFFER_SIZE * sizeof(int16_t);
  mock_ei_wrap_add_data_return_value = -EIO;

  int ret = embr_inference_move_slice_to_pipeline(pcm, pcm_size);
  zassert_equal(ret, -EIO,
                "embr_inference_move_slice_to_pipeline should return -EIO");
  zassert_equal(mock_ei_wrap_add_data_call_count, 1,
                "ei_wrap_add_data should be called once");
}

ZTEST(embr_inference,
      test_process_wos_event_audio_when_idle_returns_zero_success) {
  int ret = embr_inference_process_wos_event_audio();
  zassert_equal(ret, 0,
                "embr_inference_process_wos_event_audio should return 0");
  zassert_equal(mock_kernel_wrap_sem_take_timeout_call_count, 0,
                "kernel_wrap_sem_take_timeout_ms should not be called");
  zassert_equal(mock_thingy53_mic_read_call_count, 0,
                "thingy53_mic_read should not be called");
}
...
ZTEST(embr_inference,
      test_process_wos_event_audio_propagates_sem_error_fail) {
  int ret = embr_inference_begin_audio_capture_after_wos();
  zassert_equal(ret, 0,
                "embr_inference_begin_audio_capture_after_wos should return 0");

  mock_kernel_wrap_sem_take_timeout_return_value = -EIO;
  ret = embr_inference_process_wos_event_audio();
  zassert_equal(ret, -EIO,
                "embr_inference_process_wos_event_audio should return -EIO");
  zassert_equal(mock_thingy53_mic_read_call_count, 0,
                "thingy53_mic_read should not be called on sem error");
}

ZTEST(embr_inference,
      test_process_wos_event_audio_skips_first_slice_then_adds_data_success) {
  int ret = embr_inference_begin_audio_capture_after_wos();
  zassert_equal(ret, 0,
                "embr_inference_begin_audio_capture_after_wos should return 0");
...
ZTEST(embr_inference,
      test_process_wos_event_audio_propagates_startup_slice_read_error_fail) {
  int ret = embr_inference_begin_audio_capture_after_wos();
  zassert_equal(ret, 0,
                "embr_inference_begin_audio_capture_after_wos should return 0");

  mock_kernel_wrap_sem_take_timeout_return_value = 0;
  mock_thingy53_mic_read_return_value = -EIO;

  ret = embr_inference_process_wos_event_audio();
  zassert_equal(ret, -EIO,
                "embr_inference_process_wos_event_audio should return -EIO");
  zassert_equal(mock_ei_wrap_add_data_call_count, 0,
                "ei_wrap_add_data should not be called on read error");
}

ZTEST(embr_inference,
      test_process_wos_event_audio_keeps_skipping_when_startup_buffer_null_success) {
  int ret = embr_inference_begin_audio_capture_after_wos();
  zassert_equal(ret, 0,
                "embr_inference_begin_audio_capture_after_wos should return 0");
  ...
  ret = embr_inference_process_wos_event_audio();
  zassert_equal(ret, 0, "null startup buffer should be tolerated");
  zassert_equal(mock_ei_wrap_add_data_call_count, 0,
                "ei_wrap_add_data should not be called while still skipping");

  mock_thingy53_mic_read_buffer = pcm;
  ret = embr_inference_process_wos_event_audio();
  zassert_equal(ret, 0, "first real startup buffer should still be skipped");
  zassert_equal(mock_ei_wrap_add_data_call_count, 0,
                "ei_wrap_add_data should not be called on skipped startup data");

  ret = embr_inference_process_wos_event_audio();
  zassert_equal(ret, 0, "next buffer should be processed normally");
  zassert_equal(mock_ei_wrap_add_data_call_count, 1,
                "ei_wrap_add_data should be called after skip clears");
}

ZTEST(embr_inference,
      test_process_wos_event_audio_keeps_skipping_when_startup_size_zero_success) {
  int ret = embr_inference_begin_audio_capture_after_wos();
  zassert_equal(ret, 0,
                "embr_inference_begin_audio_capture_after_wos should return 0");
  ...
  ret = embr_inference_process_wos_event_audio();
  zassert_equal(ret, 0, "zero-size startup buffer should be tolerated");
  zassert_equal(mock_ei_wrap_add_data_call_count, 0,
                "ei_wrap_add_data should not be called while still skipping");
...
ZTEST(embr_inference,
      test_process_wos_event_audio_ignores_empty_buffer_after_skip_success) {
  int ret = embr_inference_begin_audio_capture_after_wos();
  zassert_equal(ret, 0,
                "embr_inference_begin_audio_capture_after_wos should return 0");
  ...
  mock_thingy53_mic_read_size = 0;
  ret = embr_inference_process_wos_event_audio();
  zassert_equal(ret, 0, "empty buffer after skip should be tolerated");
  zassert_equal(mock_ei_wrap_add_data_call_count, 0,
                "ei_wrap_add_data should not be called for empty buffers");
}

ZTEST(embr_inference,
      test_process_wos_event_audio_propagates_read_error_after_skip_fail) {
  int ret = embr_inference_begin_audio_capture_after_wos();
  zassert_equal(ret, 0,
                "embr_inference_begin_audio_capture_after_wos should return 0");
...
ZTEST(embr_inference,
      test_process_wos_event_audio_propagates_add_data_error_fail) {
  int ret = embr_inference_begin_audio_capture_after_wos();
  zassert_equal(ret, 0,
                "embr_inference_begin_audio_capture_after_wos should return 0");
...
ZTEST(
  ...
  ret = embr_inference_process_wos_event_audio();
  zassert_equal(ret, 0, "first invalid frame should be tolerated");

  ret = embr_inference_process_wos_event_audio();
  zassert_equal(ret, 0, "second invalid frame should be tolerated");
...
ZTEST(
  ...
  mock_thingy53_mic_read_size = expected_bytes;
  ret = embr_inference_process_wos_event_audio();
  zassert_equal(ret, 0, "valid frame should be accepted");
  zassert_equal(mock_ei_wrap_add_data_call_count, 1,
  ...
  ret = embr_inference_process_wos_event_audio();
  zassert_equal(
      ret, 0, "counter should reset: second invalid after valid is tolerated");
  ret = embr_inference_process_wos_event_audio();
  zassert_equal(
      ret, -ETIMEDOUT,
...
ZTEST(embr_inference, test_stop_audio_if_window_not_full_returns_zero_success) {
  int ret = embr_inference_stop_audio_if_window_full();
  zassert_equal(ret, 0,
                "embr_inference_stop_audio_if_window_full should return 0");
  zassert_equal(mock_thingy53_mic_stop_call_count, 0,
                "thingy53_mic_stop should not be called");
}
...
ZTEST(embr_inference, test_stop_audio_if_window_full_stops_mic_success) {
  ...
  zassert_equal(
      mock_embr_app_led_toggle_on_capture_end_async_call_count, 1,
      "embr_app_led_toggle_on_capture_end_async should be called once");
}

ZTEST(embr_inference,
      test_stop_audio_if_window_full_resets_state_after_success) {
  static int16_t pcm[CONFIG_VM3011_PDM_BUFFER_SIZE];
  size_t pcm_size = CONFIG_VM3011_PDM_BUFFER_SIZE * sizeof(int16_t);
  ...
  ret = embr_inference_stop_audio_if_window_full();
  zassert_equal(ret, 0,
                "embr_inference_stop_audio_if_window_full should return 0");
  zassert_equal(mock_thingy53_mic_stop_call_count, 1,
                "thingy53_mic_stop should not be called again");
}

ZTEST(embr_inference, test_stop_audio_if_window_full_mic_stop_error_fail) {
  static int16_t pcm[CONFIG_VM3011_PDM_BUFFER_SIZE];
  size_t pcm_size = CONFIG_VM3011_PDM_BUFFER_SIZE * sizeof(int16_t);
  mock_thingy53_mic_stop_return_value = -EIO;
  ...
  int ret = embr_inference_stop_audio_if_window_full();
  zassert_equal(ret, -EIO,
                "embr_inference_stop_audio_if_window_full should return -EIO");
  zassert_equal(
      mock_embr_app_led_toggle_on_capture_end_async_call_count, 0,
      "embr_app_led_toggle_on_capture_end_async should not be called");
  zassert_equal(mock_thingy53_mic_reset_call_count, 0,
                "thingy53_mic_reset should not be called");
}

ZTEST(embr_inference,
      test_stop_audio_if_window_full_capture_end_led_error_fail) {
  static int16_t pcm[CONFIG_VM3011_PDM_BUFFER_SIZE];
  size_t pcm_size = CONFIG_VM3011_PDM_BUFFER_SIZE * sizeof(int16_t);
  mock_embr_app_led_toggle_on_capture_end_async_return_value = -EIO;
...
ZTEST(embr_inference, test_stop_audio_if_window_full_mic_reset_error_fail) {
  static int16_t pcm[CONFIG_VM3011_PDM_BUFFER_SIZE];
  size_t pcm_size = CONFIG_VM3011_PDM_BUFFER_SIZE * sizeof(int16_t);
  mock_thingy53_mic_reset_return_value = -EIO;
