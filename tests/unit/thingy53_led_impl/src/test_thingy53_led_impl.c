...
static struct gpio_dt_spec red;
static struct gpio_dt_spec green;
static struct gpio_dt_spec blue;
...
ZTEST(thingy53_led_impl, test_init_impl_gpio_ready_and_config_success) {
  ...
  int ret = thingy53_led_init_impl(&red);
...
ZTEST(thingy53_led_impl, test_toggle_gpio_pin_toggle_success) {
  ...
  int ret = thingy53_led_toggle_impl(&red);
