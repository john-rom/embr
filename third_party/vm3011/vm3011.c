...
/* vm3011.c - VM3011 PDM Microphone Driver */
...
static inline int vm3011_get_reg(const struct device *dev,
                                 uint8_t register_adress, uint8_t *read_buf) {
  const struct vm3011_data *dmic_data = dev->data;
  return i2c_reg_read_byte(dmic_data->i2c_dev, dmic_data->i2c_address,
                           register_adress, read_buf);
}

static inline int vm3011_set_reg(const struct device *dev,
                                 uint8_t register_adress, uint8_t value) {
  const struct vm3011_data *dmic_data = dev->data;
  return i2c_reg_write_byte(dmic_data->i2c_dev, dmic_data->i2c_address,
                            register_adress, value);
}
...
static int vm3011_wdt_configure(const struct device *dev, uint8_t wdt_dly,
                                bool enable) {
...
static int vm3011_wos_pga_min_thr_set(const struct device *dev,
                                      float pga_gain_db) {
  ...
  if (ret < 0) {
    LOG_ERR("Couldn't read register %d, error %d", VM3011_REG_WOS_PGA_MIN_THR,
            ret);
    return ret;
  }
  ...
  if (ret < 0) {
    LOG_ERR("Couldn't write register %d, error %d", VM3011_REG_WOS_PGA_MIN_THR,
            ret);
    return ret;
  }
...
static int vm3011_wos_pga_max_thr_set(const struct device *dev,
                                      float pga_gain_db) {
  ...
  if (ret < 0) {
    LOG_ERR("Couldn't read register %d, error %d", VM3011_REG_WOS_PGA_MAX_THR,
            ret);
    return ret;
  }
  ...
  if (ret < 0) {
    LOG_ERR("Couldn't write register %d, error %d", VM3011_REG_WOS_PGA_MAX_THR,
            ret);
    return ret;
  }
...
static int vm3011_bpf_set(const struct device *dev, uint8_t wos_lpf_freq,
                          uint8_t wos_hpf_freq) {
  uint8_t buff;
  int ret;

  buff = (wos_lpf_freq & VM3011_WOS_BANDPASS_LPF_Msk) |
         ((wos_hpf_freq << VM3011_WOS_BANDPASS_HPF_Pos) &
          VM3011_WOS_BANDPASS_HPF_Msk);
...
static int vm3011_fast_mode_cnt_set(const struct device *dev,
  ...
  buff |=
      (fast_mode_cnt << VM3011_FAST_MODE_CNT_Pos) & VM3011_FAST_MODE_CNT_Msk;
...
static int vm3011_wos_dout_thresh_set(const struct device *dev,
                                      uint8_t wos_thresh) {
  uint8_t buff;
  int ret;
  ...
  if (ret < 0) {
    LOG_ERR("Couldn't write register %d, error %d", VM3011_REG_WOS_THRESHOLD,
            ret);
    return ret;
  }
...
static int vm3011_amb_sound_lvl_get(const struct device *dev,
                                    float *amb_sound_lvl) {
  uint8_t buff;
  int ret;
  ...
  if (ret < 0) {
    LOG_ERR("Couldn't read register %d, error %d", VM3011_REG_WOS_PGA_GAIN,
            ret);
    return ret;
  }
...
static int vm3011_chip_init(const struct device *dev) {
  ...
  ret = vm3011_wdt_configure(dev, VM3011_WDT_DELAY,
                             IS_ENABLED(CONFIG_VM3011_WDT_ENABLE));
  if (ret) {
    return ret;
  }
...
DEVICE_DT_INST_DEFINE(0, vm3011_init, NULL, &vm3011_data, &vm3011_cfg,
